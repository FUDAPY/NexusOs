...
/**
 * Aprueba un cobro pendiente: RECIEN ACA se aplica.
 *
 * POR QUE ACÁ Y NO AL REGISTRAR
 * El cobrador registra en la calle, pero el cobro no vale hasta que administracion lo revisa.
 * Este es el momento en que la deuda baja y se otorgan los puntos (1 por cada 1.000 pagados),
 * o sea el momento en que la plata se considera real.
 *
 * IDEMPOTENTE POR CONSTRUCCION
 * La condicion `deudaAplicada: false` viaja DENTRO del update que la marca, no antes como un
 * chequeo aparte: si dos administradores aprueban el mismo cobro al mismo tiempo, uno gana y
 * el otro recibe "ya fue aplicado". Aprobar dos veces NO cobra dos veces.
 *
 * SI LA DEUDA NO ALCANZA, NO SE APRUEBA
 * El descuento exige deuda suficiente en la misma operacion, y si no alcanza se cae la
 * transaccion ENTERA: el cobro vuelve a 'pendiente'. Dejarlo 'aprobado' sin descontar seria
 * plata fantasma.
 */
export const aprobarAbonoPendiente = async (
  orderId: string,
  aprobadoPor: string,
  context: { ip: string; userAgent: string },
): Promise<AprobarAbonoResult> => {
  const id = String(orderId ?? '').trim();
  if (id === '') throw new AppError('Falta el cobro', 400, 'MISSING_ABONO');

  const abono = await Order.findOne(filtroPorId(id)).exec();
  if (!abono) throw new AppError('No existe ese cobro', 404, 'ABONO_INEXISTENTE');

  if (String(abono.tipoTransaccion ?? '') !== 'abono_deuda') {
    throw new AppError('Ese ticket no es un cobro de deuda', 422, 'NO_ES_ABONO');
  }
  if (abono.deudaAplicada) {
    throw new AppError('Ese cobro ya estaba aplicado', 409, 'ABONO_YA_APLICADO');
  }
  if (String(abono.estadoAprobacionCobro ?? '') !== 'pendiente') {
    throw new AppError('Ese cobro ya fue revisado', 409, 'ABONO_YA_REVISADO');
  }

  const clienteId = String(abono.cliente ?? '').trim();
  if (clienteId === '') throw new AppError('El cobro no tiene cliente', 422, 'ABONO_SIN_CLIENTE');

  const monto = Math.round(Number(abono.total ?? 0));
  const puntosOtorgados = Math.floor(monto / PUNTOS_POR_MIL);
  const quien = String(aprobadoPor || 'Administracion');
  const sucursal = String(abono.sucursal ?? '');

  const resultado = await withTransaction(async (session) => {
    const opciones = session ? { session } : {};

    const marcado = await Order.findOneAndUpdate(
      { ...filtroPorId(id), deudaAplicada: false, estadoAprobacionCobro: 'pendiente' },
      {
        $set: {
          deudaAplicada: true,
          estadoAprobacionCobro: 'aprobado',
    ...
    if (!marcado) {
      throw new AppError('Ese cobro ya fue aplicado por otro usuario', 409, 'ABONO_YA_APLICADO');
    }

    const cliente = await User.findOneAndUpdate(
      { ...filtroPorId(clienteId), deuda: { $gte: monto - 1 } },
      { $inc: { deuda: -monto, puntos: puntosOtorgados } },
      { new: true, ...opciones },
    ).exec();
    ...
    if (!cliente) {
      throw new AppError(
        'El cliente no tiene deuda suficiente para este cobro',
        409,
        'PAGO_NO_APLICADO',
      );
    }
    ...
    await recordAudit(
      {
        tipo: 'pago_deuda',
        origen: 'pos',
        motivo: `Cobro pendiente aprobado por ${monto} (${String(abono.metodoPago ?? 'Efectivo')})`,
        sucursal,
        adminNombre: quien,
        ticketId: String(abono.ticket_id ?? ''),
        ventaId: id,
        clienteId,
        nombreCliente: String(abono.nombreCliente ?? ''),
        totalAntes: Number(cliente.deuda ?? 0) + monto,
        totalDespues: Number(cliente.deuda ?? 0),
        detalle: {
          ip: context.ip,
  ...
  emitTurnoEvent(sucursal, 'venta:creada', { turnoId: null });

  return resultado;
};

export interface RechazarAbonoResult {
  orderId: string;
  estadoAprobacionCobro: string;
}

/**
 * Rechaza un cobro pendiente. NO mueve plata: la deuda queda como estaba y no se otorgan
 * puntos. El motivo se guarda en `observacion`, que es un campo que ya existe.
 */
export const rechazarAbonoPendiente = async (
  orderId: string,
  motivo: string,
  revisadoPor: string,
): Promise<RechazarAbonoResult> => {
  const id = String(orderId ?? '').trim();
  if (id === '') throw new AppError('Falta el cobro', 400, 'MISSING_ABONO');
  ...
  const marcado = await Order.findOneAndUpdate(
    ...
    {
      $set: {
        estadoAprobacionCobro: 'rechazado',
        aprobadoPor: String(revisadoPor || 'Administracion'),
        observacion: String(motivo ?? ''),
        fechaAprobacionCobro: new Date(),
      },
    },
    { new: true },
  ...
  if (!marcado) {
    throw new AppError('Ese cobro ya fue aplicado o revisado', 409, 'ABONO_YA_REVISADO');
  }

  return { orderId: id, estadoAprobacionCobro: 'rechazado' };
};
